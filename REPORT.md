...
Full raw output is committed as `submission.json` at the repo root, generated via `adtc-profiler run --submission . --mode participant --output submission.json`. Official Gate 2 audit scores are measured independently by ADTC on the standardized evaluation machine.
